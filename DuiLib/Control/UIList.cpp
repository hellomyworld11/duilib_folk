...
namespace DuiLib {
	...
	bool CListUI::SelectItem(int iIndex, bool bTakeFocus)
	{
		...
		// 已经选择
		int aIndex = m_aSelItems.Find(iIndex);
	...
	int CListUI::GetNextSelItem(int nItem) const
	{
		...
		int aIndex = m_aSelItems.Find(nItem);
	...
	DWORD CListTextElementUI::GetTextColor(int iIndex) const
	{
		...
		DWORD dwColor = m_aTextColors.GetAt(iIndex);
		return dwColor;
	}
	...
	void CListTextElementUI::SetTextColor(int iIndex, DWORD dwTextColor)
	{
		...
		TListInfoUI* pInfo = m_pOwner->GetListInfo();
		if( iIndex < 0 || iIndex >= pInfo->nColumns ) return;
		while (m_aTextColors.GetSize() < pInfo->nColumns) { m_aTextColors.Add(pInfo->dwTextColor); }
		m_aTextColors.SetAt(iIndex, dwTextColor);
